...
use rig::{agent::Agent, client::{CompletionClient, ProviderClient, ProviderValue}, completion::{Chat, Prompt}, message::{Message, ToolCall}, providers::deepseek::{self, CompletionModel}, streaming::StreamingChat};
...
impl ChatClient {
    pub fn new(key: String, tools: Vec<McpTool>) -> Self {
        let mut builder = deepseek::Client::from_val(ProviderValue::Simple(key))
            .agent("deepseek-chat")
