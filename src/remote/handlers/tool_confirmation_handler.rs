...
impl RequestHandler for ToolConfirmationHandler {
    async fn handle(
        ...
        let InputType::ToolConfirmationResponse { name, arguments, approved, reason } = &request.input else {
            return RemoteResponse::error(&request.request_id, "Invalid request type for ToolConfirmationHandler");
        };
        ...
        if let Some(chat) = chat {
            if chat.is_waiting_tool_confirmation() {
                ...
                if let Some(error_msg) = validation_error {
                    return RemoteResponse::error(&request.request_id, &format!("Tool confirmation validation failed: {}", error_msg));
                }
                ...
                let result: Result<RemoteResponse, ()> = Ok(RemoteResponse {
                    ...
                    response: crate::remote::protocol::ResponseContent::Text(
                        format!("Tool '{}' execution was not approved by user", name)
                    ),
                    error: None,
                    token_usage: None,
                });
                ...
                match result {
                    ...
                    Err(e) => RemoteResponse::error(&request.request_id, &format!("Tool confirmation processing error: {:?}", e)),
                }
            } else {
                RemoteResponse::error(&request.request_id, "No pending tool confirmation found")
            }
        } else {
            RemoteResponse::error(&request.request_id, "No active chat session found")
        }
    }
