...
impl StateManager {
    ...
    pub fn refresh(app: &mut App) {
        ...
        match state {
            ...
            EChatState::WaitingToolConfirm => {
                Self::add_system_message_block(
                    app,
                    "检测到工具调用，是否执行？输入 yes/y 执行，no/n 取消".into(),
                );
            }
            // 等待轮次确认
            EChatState::WaitingTurnConfirm => {
                ...
                Self::add_system_message_block(
                    app,
                    format!(
                        "对话轮次已达到上限 ({}/{}), 是否继续？输入 yes/y 继续并重置计数，no/n 停止",
    ...
    pub fn add_system_message_block(app: &mut App, content: String) {
        Self::add_block(
            app,
            MessageBlock::new(ModelMessage::system(content), app.width),
        );
    }
